...
The motivation behind the package JointXshift is to understand the joint impact of intervening on multiple exposures compared to the observed outcome.
...
JointXshift takes in covariates, exposures, an outcome, a list of deltas to shift each exposure by, the estimator (tmle or one step), parallelization parameters and if the delta should be data-adaptive. 
...
```{r run JointXshift for NHANES, warning=FALSE, message=FALSE}
...
deltas <- list("LBX074LA" = 1, "LBX099LA" = 1, "LBXD03LA" = 1, "LBXD05LA" = 1, "LBXF03LA" = 1, "LBXF04LA" = 1, "LBXF08LA" = 1, "LBXPCBLA" = 1 )

NIEH_results <- JointXshift(
  w = w,
  a = a,
  y = y,
  deltas = deltas,
